...
function App() {
  ...
  const avgLatency =
    sessionMetrics.length > 0
      ? sessionMetrics.reduce((sum, m) => sum + m.latency, 0) / sessionMetrics.length
      : 0

  // Calculates average throughput by summing tokensPerSecond for all response messages and dividing by total message count
  const avgThroughput =
    sessionMetrics.length > 0
      ? sessionMetrics.reduce((sum, m) => sum + m.tokensPerSecond, 0) / sessionMetrics.length
      : 0
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: `Error: API returned ${response.status}` },
        ])
      }
    } catch (error) {
      setMessages((prev) => [
  ...
  return (
    <div className="flex h-screen bg-white text-[#363636]">
      ...
      <div className="w-72 bg-[#edede8] text-white flex flex-col">
        ...
        {/* Model selector */}
        <div className="p-4">
          <label className="text-s font-bold text-[#363636] uppercase tracking-wider">Models</label>
        ...
        <div className="px-4 mt-2">
          <h3 className="text-xs font-bold text-[#363636] uppercase tracking-wider mb-3">Session Metrics</h3>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-[#363636]">{avgThroughput.toFixed(1)} tok/s</span>
            </div>
            <div className="flex justify-between text-sm">
